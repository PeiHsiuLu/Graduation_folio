...
def is_irrelevant_comment(content):
    ...
    if len(content) < 5:  # 過短內容
        return True
    irrelevant_keywords = ["...", "哈哈", "😂", "👍", "🤔", "👌", "😅"]
    if any(keyword in content for keyword in irrelevant_keywords):  # 無意義關鍵字
        return True
...
def save_to_json(data):
    ...
    with open(file_name, 'r+', encoding='utf-8') as file:
        ...
        if data not in existing_data:
            ...
            logger.info(f"已保存文章：{data['title']}")
        else:
        ...
        for post in posts:
            ...
            if link not in processed_posts:
                ...
                if comments:
                    ...
                    logger.info(f"文章 {title} 無有效留言，略過保存。")
